    ...
    <div className="overflow-x-auto p-4 bg-gray-900 text-white min-h-screen relative">
      ...
      <div className="flex flex-col space-y-6 min-w-max">
        {currentProfile.lanes.map((lane) => {
          const laneEvents = timelineData.filter((item) => {
                ...
                {laneEvents.map((event, idx) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        const eventWithId = { ...event, id: idx };
                        setActiveEvent(eventWithId);
                        setNoteDraft(event.Note || event.note || '');
                        if (onEventClick) {
                          onEventClick(event);
                        }
                        // Also trigger attachment event for FloatingNoteConsole
                        window.dispatchEvent(
                          new CustomEvent("attachEventToNoteConsole", { detail: eventWithId })
                        );
                      }}
                      title={`${title} - ${date}`}
                    >
